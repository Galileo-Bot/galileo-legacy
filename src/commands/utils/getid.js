...
module.exports = class GetIdCommand extends Command {
	constructor() {
		super({
			name: 'getid',
			description: "Permet d'obtenir **votre** identifiant.",
			alias: ['gi'],
		});
	}
	...
	async run(client, message, args) {
		...
		message.author
			...
			.then(channel =>
				channel.send(
					`<:hey:635159039831048202> **Voici votre ID :** **\`${message.author.id}\` \n<:etiquette:635159045187174410> *Vous pouvez aussi la récupérer manuellement en passant en Mode Développeur.\n\`(Paramètres > Apparence > Mode Développeur sur PC, Paramètres > Comportement > Mode développeur sur Mobile)\`***`
				)
			)
			.catch(err => {
