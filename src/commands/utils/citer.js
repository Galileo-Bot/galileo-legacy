...
module.exports = class CiterCommand extends SlowCommand {
	...
	async run(client, message, args) {
		...
		if (args.join(' ').match(/https:\/\/(canary|ptb)?\.?discord(app)?\.com\/channels\//)) {
			...
			const channels = message.guild.channels.cache.filter(c => c.type === 'text' && c.permissionsFor(c.guild.me)
			                                                                                .has('VIEW_CHANNEL'));
		...
		if (msg.reactions.cache.size > 0) {
			embed.addField('Réactions :', msg.reactions.cache.array().map(r => `${r.emoji} : ${r.count}`).join('\n'));
		}
