...
module.exports = class ListerStaffCommand extends Command {
	...
	async run(client, message, args) {
		...
		if (admins.size > 0) embed.addField('<:inventaire:635159040510656512> Administrateurs :', admins.array().sort((a, b) => a.displayName.localeCompare(b.displayName)).join('\n'));
		if (mods.size > 0) embed.addField('<:bug:635159047284195328> Modérateurs :', mods.array().sort((a, b) => a.displayName.localeCompare(b.displayName)).join('\n'));
		if (bots.size > 0) embed.addField('<:bot:638858747351007233> Bots :', bots.array().sort((a, b) => a.displayName.localeCompare(b.displayName)).join('\n'));
