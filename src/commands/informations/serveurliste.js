...
module.exports = class ServeurListeCommand extends Command {
	...
	async run(client, message, args) {
		...
		function sortGuildsWithPage(pageNumber) {
			...
			const embedDesc = client.guilds.cache.sort((a, b) => b.memberCount - a.memberCount).map((guild) => `**${guild.name}**\t|\t**${guild.memberCount}** membres (**${guild.members.cache.filter((m) => m.user.bot).size}** bots)`).slice(pageNumber * 20 - 20, page * 20).join('\n');
