...
module.exports = class UserInfoCommand extends Command {
	...
	async run(client, message, args) {
		...
		if (message.guild) {
			...
			embed.addField('<:category:635159053298958366> Rôles', `${Util.discordSort(person.roles.cache).array().reverse().join(' | ')}\n**${person.roles.cache.size}** rôles. (**${Math.round((person.roles.cache.size / message.guild.roles.cache.size) * 100)}%** des rôles du serveur)`);
		...
		if (flags.toArray().length > 0) {
			embed.addField('Propriétés spéciales :', flags.toArray().map(flag => userFlags[flag]).sort().join('\n'));
		}
