...
module.exports = class RoleInfoCommand extends Command {
	...
	async run(client, message, args) {
		...
		embed.addField('<:richtext:635163364875698215> Nombre de membres ayant le rôle :', `${role.members.size} (${Math.round((role.members.size / message.guild.members.cache.size) * 1000) / 10}% des membres du serveur)`);
		...
		if (role.permissions.toArray().length > 0) embed.addField('Permissions', role.permissions.toArray()
		                                                                             .map(perm => permissions[perm])
		                                                                             .sort()
		                                                                             .join('\n'));
		
		await super.send(embed);
		await message.reactions.cache.find(reaction => reaction.emoji === waitEmoji)
		             .users
