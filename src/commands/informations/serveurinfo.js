...
module.exports = class ServeurInfoCommand extends Command {
	...
	async run(client, message, args) {
		...
		const {
			premiumTier, premiumSubscriptionCount, members, roles, memberCount, me, name, id, channels, createdAt, owner, features
		} = message.guild;
		...
		embed.addField('<:hey:635159039831048202> Membres :', `
...
`, true);
		...
		embed.addField('<:textuel:635159053630308391> Salons :', `
...
`, true);
		...
		if (features.length > 0) embed.addField('Fonctionnalités : ', features.map((feature) => guildFeatures[feature]).sort().join('\n'), true);
