...
module.exports = class CoupleCommand extends Command {
	...
	async run(client, message, args) {
		...
		const percentage = Math.abs(Math.round(((personOne.charCodeAt(0) * 102.4 + personTwo.charCodeAt(0) * 52.5) * personOne.length * Math.abs(Math.cos(personTwo.length) * 18.5) / new Date().getDate() * 26.2) / 56.2));
		
		await super.send(`**${personOne}** a porté son amour sur **${personTwo}**. :gift_heart:
${personOne === personTwo || personOne.toString() === personTwo.toString() ? 'Il est donc narcissique à' : 'Ses chances de réussites pour conclure sont de'} **${percentage.toString().slice(2, 4)}**% !`);
	}
};
