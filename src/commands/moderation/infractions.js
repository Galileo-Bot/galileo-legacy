	...
	async run(client, message, args) {
		...
		if (args[1] === 'supprimer') {
			if (['toutes', 'all', 'tout'].includes(args[2])) {
				if (this.client.dbManager.userInfos.get(message.guild.id, person.user.id).sanctions.find(s => s.type === 'ban') && (await message.guild.fetchBan(person))) {
					try {
						await message.guild.members.unban(person.user.id);
					} catch (ignored) {
						await super.send('Une erreur a eu lieu dans le débanissement de ce membre, vérifiez que le bot ait la permission de bannir les membres.');
					}
				}
