...
module.exports = class EmotePackCommand extends Command {
	...
	async run(client, message, args) {
		...
		if (failed.length > 0) embed.addField('⚠️ Certains émojis n\'ont pas marché !', `${failed.length} émojis n'ont pas pu être ajouté dans les ${Object.keys(emojis).length} émojis de cette commande\nVeuillez supprimer des émojis pour permettre l'ajout de ces derniers.`);
