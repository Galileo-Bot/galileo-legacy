...
module.exports = class SanctionCommand extends Command {
	...
	getPerson(message) {
		...
		if (message.member.roles.cache.map((r) => r).sort((b, a) => a.position - b.position)[0].position < person.roles.cache.map((r) => r).sort((b, a) => a.position - b.position)[0].position) {
			return argError(message, this, `Votre rôle est plus bas que la personne que vous tentez ${this.type === 'ban' ? 'de bannir' : this.type === 'kick' ? "d'éjecter" : 'd\'avertir'}, vous n'avez donc pas le droit.`);
		}
	...
	async createSanction(person) {
		...
		embed.setDescription(`${this.type === 'ban' ? 'Utilisez la commande `infractions` pour dé-bannir la personne.\n\n' : ''}Membre : ${person}\nID : ${person.user.id}\nServeur : \`${this.message.guild.name}\``);
