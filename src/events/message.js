...
module.exports = class MessageEvent extends Event {
	...
	async run(client, message) {
		...
		if (message.content === prefix) {
			return CommandManager.commands.filter(command => command.tags.includes(tags.prefix_command))
			                     .forEach(command => {
		...
		if (command) {
			...
			return CommandManager.commands.filter(command => command.tags?.includes(tags.help_command))
			                     .forEach(command => {
	...
	executeCommand(client, message, args, command) {
		...
		if (!MessageEvent.cooldown.get(message.author.id).find(c => c.command === command.name)) {
			...
			setTimeout(() => MessageEvent.cooldown.set(message.author.id, MessageEvent.cooldown.get(message.author.id)
			                                                                          .filter(c => c.command !== command.name)), command.cooldown * 1000);
		}
		
		
		command.run(client, message, args).then(() => {
	...
	logCommandExecution(message, command) {
		Logger.log(`Commande '${command}' exécutée ${message.guild ? `sur le serveur '${message.guild.name}' (${message.guild.id}) dans le salon '${message.channel.name}' (${message.channel.id})` : ' en privé '} par ${message.author.tag} (${message.author.id})`, 'MessageEvent');
	}
};
