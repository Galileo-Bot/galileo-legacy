...
function argError(message, command, error) {
	...
	if (verification.tags.length === 0 && verification.missingPermissions.user.length === 0 && verification.missingPermissions.client.length === 0) embed.addField('Rappel d\'utilisation :', `\`${command.usage}\``);
...
function permsError(message, command, missingPermissions, fromBot = false) {
	...
	embed.setTitle(`Commande : ${command.name}`);
	embed.setDescription(`\`${missingPermissions.map(perm => permissions[perm]).sort().join(', ')}\``);
